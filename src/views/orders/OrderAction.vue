...
<template>
  <div>
    ...
    <!--增加或更新的表单弹窗-->
    <el-dialog
      v-model="state.showCancelReasonModal"
      title="退款取消订单"
    >
      <textarea
    ...
    <div style="margin: 0px 10px;text-align: left;">
          ...
          <template #default="scope">
            <span v-if="scope.row.orderStatus == '未付款'">
              ...
              <el-button
                type="text"
                size="small"
                @click="cancelOrderNotPay(scope.row.orderNo)"
              >取消</el-button>
            </span>
      ...
      <el-row style="float: right">
        <el-pagination
          v-model:current-page="page.searchParams.pageNo"
          v-model:page-size="page.searchParams.pageSize"
...
const toNextOrder = (selectedOrder: orderType) => {
  ...
  if (state.selectedOrder.orderStatus === '未付款') {
    ElMessageBox.confirm('用户并未付款, 请确保已经线下付款或其他方式支付,是否继续?').then(() => {
      ElMessageBox.confirm('订单目前的状态为: ' + selectedOrder.orderStatus + '请再次确认是否继续').then(() => {
        toNextOrderStatus(selectedOrder.orderNo, selectedOrder.orderStatus).then(result => {
          ElMessage.success('订单状态已标记为: ' + result.data);
    ...
    ElMessageBox.confirm('订单目前的状态为: ' + selectedOrder.orderStatus + '是否进入下一步').then(() => {
      toNextOrderStatus(selectedOrder.orderNo, selectedOrder.orderStatus).then(result => {
        ElMessage.success('订单状态已标记为: ' + result.data);
        getOrderInfo(selectedOrder.orderStatus);
      })
...
const refundOrder = (selectedOrder: orderType) => {
  ...
  ElMessageBox.confirm('是否退款取消该订单').then(() => {
